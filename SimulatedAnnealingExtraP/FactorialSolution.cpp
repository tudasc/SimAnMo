...
/****************
Represents a model of form c_0 + c_1 * ( p!) * p^c_2
****************/
...
FactorialSolution::FactorialSolution(MeasurementDB* mdb) {
	double min_c_2 = Configurator::getInstance().min_fac_coeff_range;
	double max_c_2 = Configurator::getInstance().max_fac_coeff_range;
...
FactorialSolution FactorialSolution::getNeighborSolution() {
	...
	double pos_diff = Configurator::getInstance().max_fac_coeff_range - Configurator::getInstance().min_fac_coeff_range;
	...
	if (coeff == 2) {
		...
		do {
			...
			change = temp / 500.0;
		} while (!((val + change) >= Configurator::getInstance().min_fac_coeff_range && (val + change <= Configurator::getInstance().max_fac_coeff_range)));
		val += change;
